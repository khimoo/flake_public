# Zotero PDF の Google Drive 同期（設計判断）

設定ファイル:
- `modules/home-manager/rclone-gdrive/default.nix`（feature フラグでの条件 import）
- `modules/home-manager/rclone-gdrive/impl.nix`（同期スクリプト・scheduler・secret）
- `.sops.yaml` / `secrets/rclone.yaml`（暗号化された rclone.conf）

使い方・鍵運用の手順は [../howtouse/zotero-gdrive-sync.md](../howtouse/zotero-gdrive-sync.md) を参照。

## 全体構成

```
Zotero (linked attachments) ──> ~/Zotero-PDFs ──(watchexec 監視)──> zotero-sync
                                                                       │
                                            rclone bisync (--config 復号済み rclone.conf)
                                                                       │
                                                                Google Drive:Zotero-PDFs
```
...
`systemd.path` はディレクトリを **再帰監視できない**。Zotero-PDFs はサブフォルダを
持ちうるため、再帰監視できる watchexec を常駐サービスにする。watchexec は
...
`rclone-gdrive` は全環境共通の `homeModules` に登録するが、`default.nix` が
`settings.features.zoteroSync` が真のときだけ `sops-nix` ホームモジュールと `impl.nix`
を import する。WSL のような無効・非対応環境では sops も含め一切読み込まれない。
