...
|--------|---------|
| nixos-25.11 | 2.1.140 |
| nixos-unstable | 2.1.278 |
| 上流 anthropics/claude-code の最新リリース | 2.1.280 (2026-09-22) |
...
数日の遅れでも新しいモデルが使えないのは、クライアントが自分のカタログにないモデル ID を拒否する
ためである。2.1.268 に `--model claude-opus-5-5` を渡すと `[claude-code:unrecognized_model]` を
返して起動しない。Claude Opus 5.5 を追加したのは 2.1.280 で、その公開 (2026-09-22 15:44 UTC) から
nixpkgs master への反映 (同日 17:07 UTC) までは 1 時間半だが、nixos-unstable チャンネルへの到達には
さらに数日かかる。2.1.257 以降、nixos-unstable の claude-code は 1 日から 4 日の間隔で更新されている。
...
- 採用基準は「安定（stable）チャンネルのバージョンのままだと機能が動作しなくなり、かつ unstable 側で
  ...
  変更に追従できなくなった場合、もう1つは安定チャンネルのバージョンに既知のバグがあり、上流で修正済みの
  場合（`tinymist`）である。単に動作するバージョンが少し古いだけという理由では、ここには追加しない。
  前者に当たる `codex` と `claude-code` は、unstable でも遅れが大きいため後述のとおり専用 flake へ移した。
...
- `claude-code` も当初はこの overlay に置いていたが、`codex` と同じく専用の flake
  (`claude-code-nix`) からの取得に移した。移行前の判断は「unstable と上流の差は数日で、必要な
  バージョンに対して余裕がある」というものだったが、この余裕はモデル追加が契機である限り生じない。
  新しいモデルはクライアントのリリースと同時に選べるようになるので、必要なバージョンは常に最新版に
  なる。`codex` のような数週間の遅れではなく数日の遅れだが、その数日のあいだ新しいモデルを指定
  できないことが移行の理由である。nixpkgs が上流リリース当日に追従するようになれば overlay 経由に
  戻して input を消す。
