...
> secret（papis ライブラリ同期など）も使うマシンなら、同じ鍵を age 形式に変換して workflow repo
> （`~/sagyo/zettelkasten-workflow`）の `.sops.yaml` にも登録する。手順は [papis-gdrive-sync.md](./papis-gdrive-sync.md) を参照。

## マシンを廃棄するとき

`hosts/machines.nix` からその行を削除して各機を rebuild。以降そのマシンの鍵では
ログインできなくなる。secret を扱っていたら workflow repo（`~/sagyo/zettelkasten-workflow`）の
